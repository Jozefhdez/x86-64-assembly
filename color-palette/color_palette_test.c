...
static const uint32_t color_table[] = {
    0xFFFFFF00, 0xFFFF0000, 0x00FFFF00, 0x80008000, 0xA52A2A00, 0x1C282600,
...
extern void make_color_combination(uint32_t *combined_color,
...
uint32_t combining_function(uint32_t base, uint32_t sec) {
    ...
    for (size_t i = 0; i < 4; ++i) {
        ...
        const uint32_t channel =
            (((base >> shift) & 255) + ((sec >> shift) & 255)) & 255;
...
void test_get_color_value_cyan(void) {
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        *cyan, get_color_value(cyan),
        "The color value for cyan is different from expected.");
}

void test_get_color_value_brown(void) {

    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        *brown, get_color_value(brown),
        "The color value for brown is different from expected.");
}

void test_get_color_value_gunmetal(void) {

    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        *gunmetal, get_color_value(gunmetal),
        "The color value for gunmetal is different from expected.");
}

void test_get_color_value_byzantium(void) {

    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        *byzantium, get_color_value(byzantium),
        "The color value for byzantium is different from expected.");
}
...
void test_base_color_default(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(
        base_color, *white,
        "The value for the base color is different from expected.");
}
...
void test_constant_red(void) {
    ...
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        0xFF000000, RED,
        "The value for the constant 0xFF000000 is different from expected.");
}
...
void test_constant_green(void) {
    ...
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        0x00FF0000, GREEN,
        "The value for the constant 0x00FF0000 is different from expected.");
}
...
void test_constant_blue(void) {
    ...
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        0x0000FF00, BLUE,
        "The value for the constant 0x0000FF00 is different from expected.");
}
...
void test_combine_red_green_is_yellow(void) {
    ...
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(
        *yellow, combined_color,
        "The value of the combined color is different than expected.");
}
